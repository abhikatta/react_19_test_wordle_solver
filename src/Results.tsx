...
const Results = () => {
  ...
  const allWords = useSelector((state: RootState) => state.words.allWords);
  ...
  return allWords.length > 0 ? (
    <div className="flex flex-col items-center justify-center">
      ...
      <div className="grid grid-cols-10">
        {allWords.slice(0, showLimit ? 35 : undefined).map((word, index) => (
          <p
            key={index}
      ...
      <div className=" flex flex-row my-5 gap-10">
        {allWords.length > 35 && showLimit ? (
          <>
            <p className="  text-white text-center ">
              and {allWords.length - 35} more..
            </p>
          </>
        ) : (
          <></>
